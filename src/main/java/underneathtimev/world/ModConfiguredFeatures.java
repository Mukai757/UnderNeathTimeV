...
import underneathtimev.block.ModBlocks;
...
public class ModConfiguredFeatures {
    ...
    public static void bootstrap(BootstrapContext<ConfiguredFeature<?, ?>> context) {
        ...
        List<OreConfiguration.TargetBlockState> overworldBismuthOres = List.of(
                OreConfiguration.target(stoneReplaceables, ModBlocks.SandTimeOre.get().defaultBlockState()),
                OreConfiguration.target(deepslateReplaceables, ModBlocks.SandTimeOre.get().defaultBlockState()));
        ...
        register(context, END_UT_ORE_KEY, Feature.ORE, new OreConfiguration(endReplaceables,
                ModBlocks.SpaceDustOreBlock.get().defaultBlockState(), 9));
        register(context, END_UT_ORE_KEY, Feature.ORE, new OreConfiguration(endReplaceables,
                ModBlocks.SpaceDustOre.get().defaultBlockState(), 9));
    }

    public static ResourceKey<ConfiguredFeature<?, ?>> registerKey(String name) {
        return ResourceKey.create(Registries.CONFIGURED_FEATURE, ResourceLocation.fromNamespaceAndPath(UnderNeathTimeV.MODID, name));
    }
